...
The databricks CLI bundles top level is defined by file `mlops_employbridge_project1/databricks.yml`.
...
ML Resource Configurations in this directory:
 - model workflow (`mlops_employbridge_project1/resources/model-workflow-resource.yml`)
 - batch inference workflow (`mlops_employbridge_project1/resources/batch-inference-workflow-resource.yml`)
 - monitoring workflow (`mlops_employbridge_project1/resources/monitoring-workflow-resource.yml`)
 - feature engineering workflow (`mlops_employbridge_project1/resources/feature-engineering-workflow-resource.yml`)
 - model definition and experiment definition (`mlops_employbridge_project1/resources/ml-artifacts-resource.yml`)
...
If you want to use different catalog names, please update the `targets` declared in the `mlops_employbridge_project1/databricks.yml` and `mlops_employbridge_project1/resources/ml-artifacts-resource.yml` files.
...
|-------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------|-------------------------------------|------------------------------------------------|
| dev         | The `dev` deployment target is used by ML engineers to deploy ML resources to development workspace with `dev` configs. The config is for ML project development purposes.                                                           | dev workspace        | dev-mlops_employbridge_project1-model     | /dev-mlops_employbridge_project1-experiment     |
| staging     | The `staging` deployment target is part of the CD pipeline. Latest main content will be deployed to staging workspace with `staging` config.                                                             | staging workspace    | staging-mlops_employbridge_project1-model | /staging-mlops_employbridge_project1-experiment |
| prod        | The `prod` deployment target is part of the CD pipeline. Latest release content will be deployed to prod workspace with `prod` config.                                                                      | prod workspace       | prod-mlops_employbridge_project1-model    | /prod-mlops_employbridge_project1-experiment    |
| test        | The `test` deployment target is part of the CI pipeline. For changes targeting the main branch, upon making a PR, an integration test will be triggered and ML resources deployed to the staging workspace defined under `test` deployment target. | staging workspace    | test-mlops_employbridge_project1-model    | /test-mlops_employbridge_project1-experiment    |
...
2. Complete the `TODO` in `mlops_employbridge_project1/databricks.yml` to add the dev workspace URI under `targets.dev.workspace.host`.
...
Following this, update the batch_inference_job base parameters in `mlops_employbridge_project1/resources/batch-inference-workflow-resource.yml` to pass
...
To get started, open `mlops_employbridge_project1/resources/batch-inference-workflow-resource.yml`.  The file contains the ML resource definition of a batch inference job, like:
...
resources:
  jobs:
    batch_inference_job:
      name: ${bundle.target}-mlops_employbridge_project1-batch-inference-job
      tasks:
...
The example above defines a Databricks job with name `${bundle.target}-mlops_employbridge_project1-batch-inference-job`
that runs the notebook under `mlops_employbridge_project1/deployment/batch_inference/notebooks/BatchInference.py` to regularly apply your ML model for batch inference. 
...
We specify a `batch_inference_job` under `resources/jobs` to define a databricks workflow with internal key `batch_inference_job` and job name `{bundle.target}-mlops_employbridge_project1-batch-inference-job`.
...
`test-mlops_employbridge_project1-batch-inference-job`. During the deployment of the `staging` environment config, the job name will be
`staging-mlops_employbridge_project1-batch-inference-job`.
